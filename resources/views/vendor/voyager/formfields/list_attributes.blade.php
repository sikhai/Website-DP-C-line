...
<div class="form-group">
    ...
    @php
        ...
        // Đảm bảo $dataTypeContent không null trước khi truy cập thuộc tính
        $value = old($row->field, optional($dataTypeContent)->{$row->field} ?? '[]');
        ...
        @if ($images)
            @foreach ($images as $index => $image)
                <div class="image-item">
                    ...
                    <input type="hidden" name="{{ $row->field }}[existing][{{ $index }}][path]"
                        value="{{ $image['path'] }}">
                    <input type="text" name="{{ $row->field }}[existing][{{ $index }}][caption]"
                        value="{{ $image['caption'] ?? '' }}" class="form-control" placeholder="Nhập caption">
