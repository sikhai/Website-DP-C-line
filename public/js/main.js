...
function flip(box, index) {
    ...
    angles[index] += 180;
    let flip_query = document.querySelector('.flip-card-inner');

    if (flip_query) {
        const flip_card_inner = box.querySelector('.flip-card-inner');
        if (flip_card_inner) {
            flip_card_inner.style.transform = `rotateY(${angles[index]}deg)`;
        } else {
            console.warn("No flip card inner found in the provided box.");
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    let swiperContainer = document.querySelector('.swiper.services_list');
    if (swiperContainer){
